...
model_params = list(RODENT_ENCOUNTER_PROB=0.5,
                        ENCOUNTER_K=0.6,
                        EGG_CONTACT=3.696582, 
                        ENCOUNTER_MEAN=459.174, 
                        AGE_SUSCEPTIBILITY=0.3, 
                        CLEAR_PROB=0.2, 
                        AGE_EGG_RESISTANCE=4)

run_sim = FALSE # IF you want to run the ABC simulation
if(run_sim){
    ...
    for(method in methods){
        for(stat_set in stat_sets){
            ...
            percent_rj = 0.1
            cores = 10
            abc_fit = run_abc(steps, particles, models=c(1, 3), method=method, stat_set=stat_set, 
